...
TASKS = {
    "astrology_western": {
        "dataset": "AmareshHebbar/western_tropical-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-western-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_kp": {
        "dataset": "AmareshHebbar/kp_system-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-kp-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_horary": {
        "dataset": "AmareshHebbar/horary_prashna-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-horary-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_bazi": {
        "dataset": "AmareshHebbar/bazi-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-bazi-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_chinese_zodiac": {
        "dataset": "AmareshHebbar/chinese_zodiac-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-chinese-zodiac-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_nine_star_ki": {
        "dataset": "AmareshHebbar/nine_star_ki-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-nine-star-ki-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_numerology_pythagorean": {
        "dataset": "AmareshHebbar/numerology_pythagorean-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-numerology-pythagorean-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
    "astrology_numerology_chaldean": {
        "dataset": "AmareshHebbar/numerology_chaldean-sft",
        "model":   "unsloth/Qwen2.5-7B-Instruct-bnb-4bit",
        "output":  "AmareshHebbar/astrology-numerology-chaldean-qwen25-7b",
        "max_seq": 1024, "epochs": 2, "batch": 8, "grad_accum": 4, "lr": 2e-4, "dora": True,
    },
}
...
def preflight(tasks_to_run, dry_run):
    ...
    if not torch.cuda.is_available():
        ...
        if vram < 20:
            errors.append(f"Only {vram:.1f}GB VRAM — need 24GB+ (DoRA overhead included)")
    ...
    for t in tasks_to_run:
        cfg = TASKS[t]
        print(f"    {t:<32} {cfg['model'].split('/')[-1]:<30} batch={cfg['batch']} dora={cfg['dora']}")
...
def train_task(task_name, results, task_idx, total_tasks, t_start):
    ...
    wandb.init(project="astrology-divination", name=f"{task_name}-qdora", reinit="finish_previous")

    print(f"[3/5] Training — {cfg['epochs']} epochs  batch={cfg['batch']}x{cfg['grad_accum']}  dora={cfg['dora']}")
    out_dir = WORKSPACE / f"{task_name}-qdora"
...
def print_summary(results, t_start):
    ...
    for r in results:
        ...
        print(f"  {r['task']:<32} {r['time_hr']:>5.1f}h  {status}")
