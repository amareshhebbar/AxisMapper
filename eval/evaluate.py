...
for i, ex in enumerate(tqdm(test_data, desc="Evaluating", unit="scenario")):
    messages  = ex["messages"]
    expected  = next(m["content"] for m in messages if m["role"] == "assistant")
    exp_codes = set(c.strip() for c in expected.split(","))
    ...
    pred_str   = tokenizer.decode(out[0][inputs["input_ids"].shape[1]:],
                                  skip_special_tokens=True).strip()
    pred_codes = set(c.strip() for c in pred_str.split(",") if c.strip())
